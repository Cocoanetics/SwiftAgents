...
enum ApplyDiffError: LocalizedError {
...
private let SECTION_TERMINATORS = [END_PATCH, "*** Update File:", "*** Delete File:", "*** Add File:"]
private let END_SECTION_MARKERS = SECTION_TERMINATORS + [END_FILE]

// MARK: - Public API

func applyDiff(input: String, diff: String, mode: ApplyDiffMode = .default) throws -> String {
...
private func isDone(_ state: ParserState, prefixes: [String]) -> Bool {
    ...
    return prefixes.contains(where: { state.lines[state.index].hasPrefix($0) })
}
...
private func parseCreateDiff(_ lines: [String], newline: String) throws -> String {
    ...
    while !isDone(parser, prefixes: SECTION_TERMINATORS) {
        ...
        parser.index += 1
        guard line.hasPrefix("+") else { throw ApplyDiffError.invalidAddFileLine(line) }
...
private func parseUpdateDiff(_ lines: [String], input: String) throws -> [DiffChunk] {
    ...
    while !isDone(parser, prefixes: END_SECTION_MARKERS) {
        ...
        let hasBareAnchor = anchor.isEmpty && parser.index < parser.lines.count && parser.lines[parser.index] == "@@"
        ...
        if findResult.newIndex == -1 {
            ...
            if section.eof { throw ApplyDiffError.invalidEOFContext(cursor, ctxText) }
        ...
        for ch in section.sectionDiffChunks {
            chunks.append(DiffChunk(origIndex: ch.origIndex + findResult.newIndex, delLines: ch.delLines, insLines: ch.insLines))
        }
    }
    return chunks
}
...
private func advanceCursorToAnchor(_ anchor: String, inputLines: [String], cursor: Int, parser: ParserState) -> Int {
    ...
    if !inputLines[0..<cursor].contains(where: { $0 == anchor }) {
        for i in cursor..<inputLines.count {
            if inputLines[i] == anchor { cursor = i + 1; found = true; break }
        }
    }
    let trimmed = anchor.trimmingCharacters(in: .whitespaces)
    if !found && !inputLines[0..<cursor].contains(where: { $0.trimmingCharacters(in: .whitespaces) == trimmed }) {
        for i in cursor..<inputLines.count {
            if inputLines[i].trimmingCharacters(in: .whitespaces) == trimmed {
                cursor = i + 1; parser.fuzz += 1; break
            }
        }
    }
    return cursor
...
private func readSection(_ lines: [String], startIndex: Int) throws -> ReadSectionResult {
    var context = [String](), delLines = [String](), insLines = [String]()
    var sectionDiffChunks = [DiffChunk]()
    ...
    while index < lines.count {
        let raw = lines[index]
        if raw.hasPrefix("@@") || raw.hasPrefix(END_PATCH) || raw.hasPrefix("*** Update File:")
            || raw.hasPrefix("*** Delete File:") || raw.hasPrefix("*** Add File:") || raw.hasPrefix(END_FILE) { break }
        if raw == "***" { break }
        if raw.hasPrefix("***") { throw ApplyDiffError.invalidLine(raw) }
        ...
        case "-": mode = .delete
        case " ": mode = .keep
        default: throw ApplyDiffError.invalidLine(line)
        }
        let lineContent = String(line.dropFirst())
        if mode == .keep && lastMode != .keep && (!delLines.isEmpty || !insLines.isEmpty) {
            sectionDiffChunks.append(DiffChunk(origIndex: context.count - delLines.count, delLines: delLines, insLines: insLines))
            delLines = []; insLines = []
        }
        switch mode {
        case .delete: delLines.append(lineContent); context.append(lineContent)
        case .add: insLines.append(lineContent)
        case .keep: context.append(lineContent)
        }
    }
    ...
    if index == origIndex {
        throw ApplyDiffError.emptySection(index, index < lines.count ? lines[index] : "")
    }
...
private func findContextCore(_ lines: [String], context: [String], start: Int) -> ContextMatch {
    guard !context.isEmpty else { return ContextMatch(newIndex: start, fuzz: 0) }
    for i in start..<lines.count {
        if equalsSlice(lines, target: context, start: i, map: { $0 }) { return ContextMatch(newIndex: i, fuzz: 0) }
    }
    for i in start..<lines.count {
        if equalsSlice(lines, target: context, start: i, map: { $0.replacingOccurrences(of: "\\s+$", with: "", options: .regularExpression) }) { return ContextMatch(newIndex: i, fuzz: 1) }
    }
    for i in start..<lines.count {
        if equalsSlice(lines, target: context, start: i, map: { $0.trimmingCharacters(in: .whitespaces) }) { return ContextMatch(newIndex: i, fuzz: 100) }
    }
...
private func applyDiffChunks(_ input: String, chunks: [DiffChunk], newline: String) throws -> String {
    ...
    var cursor = 0
    for chunk in chunks {
        guard chunk.origIndex <= origLines.count else { throw ApplyDiffError.chunkOutOfBounds(chunk.origIndex, origLines.count) }
        guard cursor <= chunk.origIndex else { throw ApplyDiffError.overlappingDiffChunk(chunk.origIndex, cursor) }
