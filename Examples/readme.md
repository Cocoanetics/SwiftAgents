...
- **Session modes** (`session/set_mode`) — **`code`** (full tool access) vs. **`plan`** (read-only: `bash`, `write`, `edit`, and `apply_patch` are withheld/refused, and the client's MCP servers are withheld too, so the agent explores and proposes a plan without touching the working tree). A switch is confirmed with a `current_mode_update`.
- **Config options** (`session/set_config_option`) — a `reasoning_effort` select (`auto` / `low` / `medium` / `high`) for reasoning-capable models.
- **Resumable sessions** (`session/load`) — every session persists to `~/.coder/acp-sessions/<id>.json` (working directory, model/mode/effort, the rolling `lastResponseId`, and a short transcript). A client can reconnect after the Coder process restarts: Coder restores the state, reconnects the MCP servers the client re-supplies, and replays the transcript. Whether the *model* keeps its context across the restart depends on the provider — for one with server-side history (OpenAI Responses, LM Studio) the stored `lastResponseId` resumes the thread (the default `gpt-5.4` does this); for a stateless provider (Anthropic, Gemini, Ollama) the transcript is shown for reference but Coder tells the client the model starts fresh.
