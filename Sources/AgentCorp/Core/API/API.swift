...
extension URL
{
	...
	public static let googleAI = URL(string: "https://generativelanguage.googleapis.com")!
}

public class API: @unchecked Sendable
{
	internal let apiKey: String?
	internal let endpointURL: URL
	...
	public lazy var encoder = {
		...
		encoder.keyEncodingStrategy = JSONEncoder.KeyEncodingStrategy.custom { codingPath in
			...
			if codingPath.contains(where: { ["schema", "tools"].contains($0.stringValue)}) {
				...
				let snake = last.stringValue.replacingOccurrences(of: "([a-z0-9])([A-Z])", with: "$1_$2", options: .regularExpression).lowercased()
	...
	public var streamingChunkHandler: (Chunk)->() = { chunk in
	...
	internal func errorFromResponse(data: Data, response: HTTPURLResponse) -> Error
	{
		do {
	 ...
	 */
	internal func process<C: Codable>(data: Data, response: URLResponse) throws -> C
	{
	...
	internal func process(data: Data, response: URLResponse) throws -> Data
	{
		guard let response = response as? HTTPURLResponse else
		{
			throw APIError.invalidResponse
	 ...
	 */
	fileprivate func process(asyncBytes: URLSession.AsyncBytes, response: URLResponse) async throws -> ChatCompletionResponse
	{
		guard let response = response as? HTTPURLResponse else
		{
			throw APIError.invalidResponse
		...
		for try await line in asyncBytes.lines
		{
			...
			do {
				...
				if chatReponse == nil
				{
					...
					let message = ChatMessage(role: chunkChoice.delta.role ?? .assistant, content: chunkChoice.delta.content.map { .text($0) })
						return ChatCompletionResponse.Choice(message: message, finishReason: chunkChoice.finishReason, index: chunkChoice.index)
					}
					
					chatReponse = ChatCompletionResponse(id: chunk.id, object: "chat.completion",
														 created: chunk.created, model: chunk.model, usage: nil, choices: choices)
				}
				else
				{
			...
			if let deltaMsg = delta.content
			{
				...
				message.content = .text(existing + deltaMsg)
						...
						return ChatCompletionResponse.Choice(message: message, finishReason: choice.finishReason ?? chunkChoice.finishReason, index: choice.index)
					}
		...
		let updatedChoices: [ChatCompletionResponse.Choice] = chatReponse.choices.map { oldChoice in
			...
			return ChatCompletionResponse.Choice(message: updatedMessage, finishReason: oldChoice.finishReason, index: oldChoice.index)
		}
	 ...
	 */
	internal func processStream(asyncBytes: URLSession.AsyncBytes, response: URLResponse) async throws -> AsyncThrowingStream<Chunk, Error>
	{
		guard let response = response as? HTTPURLResponse else
		{
			throw APIError.invalidResponse
