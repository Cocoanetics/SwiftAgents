...
extension OpenAI {
									  ...
									  include: [Include]? = nil) async throws -> ConversationItemList
	{
		...
		let request = try self.createUrlRequest(path: "/v1/conversations/\(conversationId)/items",
										 ...
										 include: [Include]? = nil) async throws -> ConversationItem
	{
		...
		let request = try self.createUrlRequest(path: "/v1/conversations/\(conversationId)/items/\(itemId)",
