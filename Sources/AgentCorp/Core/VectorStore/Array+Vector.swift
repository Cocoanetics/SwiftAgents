...
extension Sequence where Element == Double {
	...
	/// - Returns: The cosine similarity value ranging from -1 to 1, where 1 means the vectors are identical in orientation.
	public func cosineSimilarity(to otherVector: Vector) -> Double {
		let array = Array(self)
	...
	///
	/// When dealing with unit vectors (vectors of magnitude 1), the cosine similarity simplifies to just the dot product.
	...
	public func cosineSimilarityForUnitVector(to otherUnitVector: Vector) -> Double {
		let array = Array(self)
	...
	/// Both vectors must have the same number of elements; otherwise, the maximum possible double value is returned to indicate error.
	///
	/// - Parameter otherVector: The vector to which the Euclidean distance will be calculated.
	/// - Returns: The Euclidean distance between the two vectors.
	public func euclideanDistance(to otherVector: Vector) -> Double {
		let array = Array(self)
...
extension Array where Element == Vector {
	...
	///
	/// - Returns: A vector representing the element-wise average of all contained vectors, or nil if vectors have different lengths or the array is empty.
	public func averageVector() -> Vector? {
		guard !self.isEmpty, let firstLength = self.first?.count else {
			return nil
		}
		
		// Ensure all vectors are of the same length
		guard self.allSatisfy({ $0.count == firstLength }) else {
			return nil
		}
	...
	public func averageUnitVector() -> Vector? {
		guard let avgVector = self.averageVector() else {
			return nil
		}
	...
	/// All vectors must have the same number of elements; otherwise, `nil` is returned.
	///
	/// - Returns: A vector representing the sum of all contained vectors, or nil if vectors have different lengths or the array is empty.
	public func sumVector() -> Vector? {
		guard !self.isEmpty, let firstLength = self.first?.count else {
			return nil
		}
	...
	public func sumUnitVector() -> Vector? {
		guard let sumVector = self.sumVector() else {
			return nil
		}
