...
/** Main runner for executing agent workflows
 ...
 Executes the agent loop — model call, tool calls, repeat — against the
 configured LLM provider, resolved from the model string via the shared
 `ProviderRegistry` (OpenAI, Anthropic, Google, Ollama, LM Studio, …) or
 injected directly through `RunConfig.api`.
 */
public actor Runner {
    ...
    // Helper function for executing agent turns. Internal (not private)
    // because the input-guardrail fan-out in `Runner+Guardrails.swift`
    // drives the same turn loop.
    static func executeAgentTurns<A: Agent>(
        agent: A,
    ...
    // live in `Runner+Helpers.swift`, and the input-guardrail fan-out
    // (`executeWithInputGuardrails` and its carrier types) in
    // `Runner+Guardrails.swift`, to keep this actor body focused on the
