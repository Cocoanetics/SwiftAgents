	...
	func run() async throws {
		...
		terminal.handleInput = { input in
			...
			Task {
				...
				let result = Runner.runStreamed(agent: agent, input: input, maxTurns: 50, previousResponseId: lastResponseId, config: config)
				...
				do {
					...
					for try await event in result.events {
						switch event {
						case .rawResponseEvent(let e):
							switch e.object {
							case .reasoningTextDelta(let info):
								let text = info.delta
							...
							case .outputTextDelta(let info):
								...
								if !delta.isEmpty {
									lineBuffer += delta
									lineBuffer = flushMarkdown(lineBuffer, terminal: terminal, inCodeBlock: &inCodeBlock)
								}
						...
						case .runItemEvent(let name, let item):
							if case .toolOutput = name, case .toolOutput(let callId, let output) = item {
								sessionLogger.log(.toolResult(ToolResultEntry(callId: callId, output: output)))
							}
							if case .toolCalled = name, case .toolCall(let toolName, let args, let callId) = item {
								sessionLogger.log(.toolCall(ToolCallEntry(name: toolName, arguments: args, callId: callId)))
					...
					for response in completedResponses {
						for outputItem in response.output {
							if case .message(let msg) = outputItem {
								let text = msg.content.compactMap { if case .outputText(let t) = $0 { t.text } else { nil } }.joined()
								if !text.isEmpty {
