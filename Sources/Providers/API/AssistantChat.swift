...
public class AssistantChat: GenericChat {
	...
	public init(api: OpenAI, model: String, systemPrompt: String, temperature: Double = 0.7, stream: Bool = false, tools: [MCPToolProviding]? = nil) {
		self.api = api
		self.model = model
		self.tools = tools
	...
	public init(api: OpenAI, chatCompletionRequest: ChatCompletionRequest, tools: [MCPToolProviding]? = nil) {
		self.api = api
		self.model = chatCompletionRequest.model
		self.tools = tools
		self.stream = chatCompletionRequest.stream ?? true
		self.temperature = chatCompletionRequest.temperature ?? 1

		var messages = chatCompletionRequest.messages

		if let index = messages.firstIndex(where: { $0.role == .system }) {
			self.systemPrompt = messages[index].textContent ?? ""
			messages.remove(at: index)
		} else {
			self.systemPrompt = "You are a helpful assistent"
		}
	...
	public func setupIfNecessary(assistantID: String? = nil, threadID: String? = nil) async throws {
		...
		if let assistantID = assistantID {
			...
			if let tools {
				toolDescriptions += tools.flatMap { $0.toolDescriptions }
			}

			assistant = try await api.createAssistant(model: model, instructions: systemPrompt, tools: toolDescriptions, temperature: temperature)
		}
