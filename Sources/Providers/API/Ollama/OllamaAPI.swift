...
public class OllamaAPI: API, @unchecked Sendable {
	...
	override public func createChatCompletion(model: String,
		...
		let choices = response.choices.map { choice in
			...
			return ChatCompletionResponse.Choice(message: modifiedMessage, finishReason: .toolCalls, index: choice.index)
		}
	...
	public lazy var ollamaDecoder = {
		...
		decoder.dateDecodingStrategy = .custom { decoder -> Date in
			...
			formatter.dateFormat = "yyyy-MM-dd'T'HH:mm:ss.SSSSSSSSSXXXXX" // ISO 8601 format with milliseconds and timezone offset
	...
	public override func models() async throws -> [Model] {
		...
		do {
			...
			return listResponse.models.map { ollamaModel in
				...
				return Model(id: ollamaModel.name, object: nil, created: ollamaModel.modifiedAt, ownedBy: nil, permission: nil, root: nil, parent: nil)
			}
	...
	fileprivate func inject(tools: [ToolDescription]?, messages: [ChatMessage]) -> [ChatMessage] {
		...
		let modifiedMessages = messages.map { message in
			...
			let functions = toolsWithFunction.compactMap({ $0.function })
