...
extension Runner {
	...
	public static func runStreamed<A: Agent>(
		agent: A,
		input: String,
		maxTurns: Int = 10,
			...
			let result: AgentResult<A.OutputType> = try await withSpan { agentSpan in
				...
				for proxy in currentAgent.mcpServers {
					tools += try await withSpan { mcpListSpan in
						...
						mcpListSpan.spanData = MCPListToolsSpanData(server: serverName, result: mcpTools.map { $0.name })
						return myTools
					}
				}
		...
		while turns < maxTurns {
				...
				for try await event in stream {
					...
					case .outputItemDone(let info):
						...
						case .applyPatchCall(let patchCall):
							// Notify caller that a patch call is in progress (diff may be incomplete from streaming)
							...
							continuation.yield(.runItemEvent(
								name: .toolCalled,
								item: .toolCall(name: "apply_patch", arguments: patchCall.operation.path, callId: patchCall.resolvedCallId)
							))
			...
			if let response = completedResponse {
				for outputItem in response.output {
					if case .applyPatchCall(let patchCall) = outputItem {
			...
			if !toolOnlyCalls.isEmpty {
				...
				for result in results {
					if case .functionCallOutput(let output) = result {
						continuation.yield(.runItemEvent(
							name: .toolOutput,
