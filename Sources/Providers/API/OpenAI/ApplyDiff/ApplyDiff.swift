...
public enum ApplyDiffError: LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
            case let .invalidAddFileLine(line): return "Invalid Add File Line: \(line)"
            case let .invalidLine(line): return "Invalid Line: \(line)"
            case let .invalidContext(cursor, ctx): return "Invalid Context \(cursor):\n\(ctx)"
