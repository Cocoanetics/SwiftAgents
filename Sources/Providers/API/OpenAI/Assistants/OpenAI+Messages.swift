...
extension OpenAI {
    ...
    func createMessage(
        ...
        let request = try createUrlRequest(httpMethod: "POST", path: "/v1/threads/\(threadId)/messages", body: details)
    ...
    func listMessages(
        ...
        before: String? = nil,
        runId: String? = nil
    ) async throws -> ListPagedResponse<Message> {
        ...
        let request = try createUrlRequest(path: "/v1/threads/\(threadId)/messages", queryItems: queryItems)
    ...
    func retrieveMessage(threadId: String, messageId: String) async throws -> Message {
        let request = try createUrlRequest(path: "/v1/threads/\(threadId)/messages/\(messageId)")
    ...
    func deleteMessage(threadId: String, messageId: String) async throws -> Bool {
        let request = try createUrlRequest(httpMethod: "DELETE", path: "/v1/threads/\(threadId)/messages/\(messageId)")
