...
public extension OpenAI {
    ...
    func generateImage(
        ...
        let maxPromptLength = switch model {
            case .dalle2: 1000
            case .dalle3: 4000
            case .gptImage: 32000
        }
        guard prompt.count <= maxPromptLength else {
            throw ImageGenerationError
                .invalidParameter(
                    "Prompt exceeds maximum length of \(maxPromptLength) characters for \(model.identifier)"
                )
        }
        ...
        switch model {
            case let .dalle2(options):
                // DALL-E 2 specific checks
                guard options.count >= 1, options.count <= 10 else {
                    throw ImageGenerationError
                        .invalidParameter("DALL-E 2 requires 1 to 10 images (count=\(options.count))")
                }
            ...
            case let .gptImage(options):
                // GPT Image specific checks
                guard options.count >= 1, options.count <= 10 else {
                    throw ImageGenerationError
                        .invalidParameter("GPT Image requires 1 to 10 images (count=\(options.count))")
                }
