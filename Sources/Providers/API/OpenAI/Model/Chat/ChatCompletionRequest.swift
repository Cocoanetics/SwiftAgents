...
public struct ChatCompletionRequest: Codable, Sendable {
    ...
    /// while lower values like 0.2 will make it more focused and deterministic. (Optional, defaults to 1)
    public let temperature: Double?

    /// Up to 4 sequences where the API will stop generating further tokens. (Optional)
    public let stop: [String]?

    /// Whether or not to store the output of this chat completion request for use in our model distillation or evals
    /// products.
        ...
        public init(from decoder: Decoder) throws {
            ...
            if let includeUsage = try container.decodeIfPresent(Bool.self, forKey: .includeUsage) {
                ...
                throw DecodingError.dataCorruptedError(
                    forKey: .includeUsage,
                    in: container,
                    debugDescription: "Invalid stream options"
                )
            }
        }
    }
