...
public enum OutputItem: Codable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        switch type {
            case "message":
                self = try .message(MessageOutput(from: decoder))
            case "file_search_call":
                self = try .fileSearch(FileSearchOutput(from: decoder))
            case "function_call":
                self = try .functionCall(FunctionCall(from: decoder))
            case "web_search_call":
                self = try .webSearch(WebSearchOutput(from: decoder))
            case "computer_call":
                self = try .computer(ComputerOutput(from: decoder))
            case "reasoning":
                self = try .reasoning(ReasoningOutput(from: decoder))
            case "mcp_list_tools":
                self = try .mcpListTools(MCPListToolsOutput(from: decoder))
            case "mcp_approval_request":
                self = try .mcpApprovalRequest(MCPApprovalRequestOutput(from: decoder))
            case "mcp_call":
