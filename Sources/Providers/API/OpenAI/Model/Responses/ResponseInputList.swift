...
public enum ResponseListItem: Codable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let type = try? container.decode(String.self, forKey: .type) {
            switch type {
                case "message":
                    if let role = try? container.decode(Role.self, forKey: .role) {
                        if role == .user || role == .system || role == .developer {
                            self = try .inputMessage(InputMessage(from: decoder))
                        } else if role == .assistant {
                            self = try .outputMessage(OutputItem.MessageOutput(from: decoder))
                        } else {
                            throw DecodingError.dataCorruptedError(
                                forKey: .role,
                                in: container,
                                debugDescription: "Invalid role: \(role)"
                            )
                        }
                    } else {
                        throw DecodingError.keyNotFound(
                            CodingKeys.role,
                            DecodingError.Context(codingPath: decoder.codingPath, debugDescription: "Missing role")
                        )
                    }
                ...
                case "function_call":
                    self = try .functionCall(OutputItem.FunctionCallOutput(from: decoder))
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        switch self {
            ...
            case let .computerCallOutput(output):
            ...
            case let .itemReference(reference):
