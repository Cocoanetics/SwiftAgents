...
public enum RealtimeConversationItem: Codable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        switch type {
            ...
            case "mcp_list_tools":
                self = try .mcpListTools(OutputItem.MCPListToolsOutput(from: decoder))
            case "mcp_approval_request":
                self = try .mcpApprovalRequest(OutputItem.MCPApprovalRequestOutput(from: decoder))
            case "mcp_call":
                self = try .mcpCall(OutputItem.MCPCallOutput(from: decoder))
            default:
    ...
    public struct Message: Codable, Sendable {
        public enum Content: Codable, Sendable {
            ...
            public init(from decoder: Decoder) throws {
                ...
                switch type {
                    case "input_text":
                        self = try .inputText(InputText(from: decoder))
                    case "input_audio":
                        self = try .inputAudio(Audio(from: decoder))
                    case "output_text":
                        self = try .outputText(OutputText(from: decoder))
                    case "output_audio":
                        self = try .outputAudio(Audio(from: decoder))
                    default:
                        throw DecodingError.dataCorruptedError(
                            forKey: .type,
                            in: container,
                            debugDescription: "Unsupported realtime message content type: \(type)"
                        )
                }
            }
        ...
        public struct OutputText: Codable, Sendable {
            ...
            public init(from decoder: Decoder) throws {
                ...
                annotations = try container.decodeIfPresent(
                    [OutputItem.MessageOutput.Annotation].self,
                    forKey: .annotations
                ) ?? []
