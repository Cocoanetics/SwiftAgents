...
public actor RealtimeSession {
	...
	public init(
		...
		self.history = RealtimeHistory()
		self.currentAgent = agent
		self.model = model
		self.initialModelIdentifier = modelIdentifier ?? agent.model ?? "gpt-realtime"
	}
	...
	public func rebind(to agent: any RealtimeAgent) async throws {
		...
		guard requestedModel == initialModelIdentifier else {
			throw RealtimeSessionError.modelChangeRequiresNewSession(current: initialModelIdentifier, requested: requestedModel)
		}
	...
	private func resolvedSessionConfiguration(for agent: any RealtimeAgent) async throws -> RealtimeSessionConfiguration {
	...
	private func resolvedRealtimeTools(for agent: any RealtimeAgent) async throws -> [Tool]? {
		...
		let handoffToolNames = Set(agent.handoffs.map { $0.toolName })
		...
		for proxy in agent.mcpServers {
			...
			for listedTool in listedTools {
				...
				if case .object(let object, _) = listedTool.inputSchema {
	...
	private func executeAndContinue(_ functionCalls: [OutputItem.FunctionCall], using agent: any RealtimeAgent) async {
		...
		do {
			...
			for output in outputs {
				if case .functionCallOutput(let functionCallOutput) = output {
		...
		try await model.send(RealtimeResponseCreateEvent())
			...
			lastServerError = error
			pendingResponseRequests = max(0, pendingResponseRequests - 1)
			let detail = ErrorDetail(message: error.localizedDescription, type: "tool_execution_error", param: nil, code: nil)
