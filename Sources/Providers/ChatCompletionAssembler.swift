...
public class ChatCompletionAssembler
{
	...
	var choices: [ChatCompletionResponse.Choice] 
	{
		return currentChoices.sorted(by: { $0.key < $1.key }).map { internalChoice in
			...
			let message = ChatMessage(role: choice.role ?? .assistant, content: choice.content.map { .text($0) }, toolCalls: !choice.toolCalls.isEmpty ? choice.toolCalls : nil )
	...
	fileprivate struct InternalChoice 
	{
		...
		{
			return toolCallsDict.sorted(by: { $0.key < $1.key }).map { $0.value }
		}
	}
	
	/// A dictionary of current choices indexed by their choice index.
	fileprivate var currentChoices: [Int: InternalChoice] = [:]
	...
	public func processChunk(_ chunk: Chunk) 
	{
		...
		for choice in chunk.choices 
		{
			...
			{
				var newChoice = InternalChoice(index: choice.index, finishReason: choice.finishReason, role: choice.delta.role, content: choice.delta.content)
