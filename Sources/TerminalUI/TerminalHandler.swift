...
public class TerminalHandler {
	...
	private static weak var activeHandler: TerminalHandler?
				...
				if escapeSequenceActive {
					...
					if escapeBuffer.count == 3 {  // Length of typical arrow key sequence
						if escapeBuffer[0] == 27 && escapeBuffer[1] == 91 {  // ESC + [
							if escapeBuffer[2] == 65 || escapeBuffer[2] == 66 || escapeBuffer[2] == 67 || escapeBuffer[2] == 68 {  // A, B, C, D
	...
	private func deleteWord() {
		...
		while inputBuffer.last?.isWhitespace == true { inputBuffer.removeLast() }
		// Then remove until next whitespace or empty
		while !inputBuffer.isEmpty && inputBuffer.last?.isWhitespace != true { inputBuffer.removeLast() }
	}
