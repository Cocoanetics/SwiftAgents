...
struct VectorStoreLifecycleTests {
	...
	func vectorStoreFileLifecycle() async throws {
		do {
			...
			let file = try await openAI.uploadFile(url: fixture, filename: "vector-\(UUID().uuidString.prefix(6)).txt", purpose: .assistants)
			let store = try await openAI.createVectorStore(name: "Test \(UUID().uuidString.prefix(6))", metadata: ["scope": "swift-testing"], fileIds: [file.id])

			let processedFile = try await openAI.waitUntilVectorStoreFileIsReady(vectorStoreId: store.id, fileId: file.id)
	...
	func vectorStoreFileBatch() async throws {
		let openAI = try TestClients.openAI()
		let store = try await openAI.createVectorStore(name: "Batch \(UUID().uuidString.prefix(6))", metadata: ["scope": "swift-testing"])
		
		let resources = [
			try TestResources.url(forResource: "pasching", withExtension: "txt"),
			try TestResources.url(forResource: "vision", withExtension: "txt")
		]
		...
		do {
			let batch = try await openAI.createVectorStoreFilesBatch(vectorStoreId: store.id, fileIds: uploaded.map(\.id))
			let readyBatch = try await openAI.waitUntilVectorStoreBatchIsReady(vectorStoreId: store.id, batchId: batch.id)
