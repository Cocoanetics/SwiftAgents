...
struct GoogleImageGenerationTests {
    ...
    func generatesWideImage() async throws {
        ...
        let response = try await google.createChatCompletion(
            ...
            messages: [.init(
                role: .user,
                content: .text("Draw a cinematic 16:9 postcard of a penguin astronaut waving from a moon base.")
            )],
    ...
    func generatesSquareImage() async throws {
        ...
        let response = try await google.createChatCompletion(
            ...
            messages: [.init(
                role: .user,
                content: .text("Generate a crisp 2K square icon of a sleepy robot drinking tea.")
            )],
    ...
    private func assertImageHasAspect(
        ...
        #expect(
            ...
            "Unexpectedly small image: \(Int(width))x\(Int(height))"
        )
    }
}
