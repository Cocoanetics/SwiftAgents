...
struct OllamaChatTests {
	...
	@Test("Ollama chat completion", .enabled(if: Self.runOllamaTests && TestClients.hasOllama, "Requires OLLAMA_URL and RUN_OLLAMA_TESTS=1"))
	func chatCompletion() async throws {
		...
		let completion = try await ollama.createChatCompletion(model: "llama3", messages: [systemPrompt, userMessage], n: 1)
	...
	@Test("Ollama chat streaming", .enabled(if: Self.runOllamaTests && TestClients.hasOllama, "Requires OLLAMA_URL and RUN_OLLAMA_TESTS=1"))
	func chatStreaming() async throws {
		let ollama = try TestClients.ollama()
		let systemPrompt = ChatMessage(role: .system, content: .text("You are a helpful and witty assistant"))
		let userMessage = ChatMessage(role: .user, content: .text("Tell me a joke"), name: "Xcode")

		let stream = try await ollama.createChatCompletionStream(model: "llama3", messages: [systemPrompt, userMessage], n: 1)
	...
	@Test("Ollama function calling", .enabled(if: Self.runOllamaTests && TestClients.hasOllama, "Requires OLLAMA_URL and RUN_OLLAMA_TESTS=1"))
