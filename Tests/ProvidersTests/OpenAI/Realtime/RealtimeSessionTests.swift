	...
	var mcpToolMetadata: [MCPToolMetadata] {
		[
			MCPToolMetadata(
				...
				parameters: [
					MCPParameterInfo(name: "lhs", type: Double.self, description: "Left-hand number.", isRequired: true),
					MCPParameterInfo(name: "rhs", type: Double.self, description: "Right-hand number.", isRequired: true)
				],
...
private actor MockRealtimeModel: RealtimeModel {
	...
	func send<Event: Encodable & Sendable>(_ event: Event) async throws {
		sentEvents.append(try encoder.encode(event))
	}

	func sendRaw(_ payload: [String: JSONValue]) async throws {
		sentEvents.append(try encoder.encode(payload))
	}
