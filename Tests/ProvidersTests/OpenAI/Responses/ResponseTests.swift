...
struct ResponseTests {
	...
	func responseRequestEncoding() throws {
		...
		let requestBody = ResponseOptionals(
			...
			model: "gpt-5.3-codex",
			contextManagement: [.init(compactThreshold: 4_096)],
			conversation: nil,
	...
	func responseStream() async throws {
		...
		for try await event in stream {
			events.append(event)
			if case .responseCompleted(let response) = event.object {
	...
	private func calculatorTools() -> [Tool] {
		Calculator().toolDescriptions.compactMap { description in
			...
			return .function(FunctionTool(name: function.name, description: function.description, parameters: function.parameters, strict: true))
		}
	}
}
