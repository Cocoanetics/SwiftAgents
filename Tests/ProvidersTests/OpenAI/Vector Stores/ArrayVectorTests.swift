...
struct ArrayVectorTests {
    ...
    func cosineSimilarityForUnitVectors() throws {
        let unit = try #require(Vector([1.0, 0.0, 0.0]).unitVector())
        #expect(unit.cosineSimilarityForUnitVector(to: unit) == 1)

        let perpendicularA = try #require(Vector([1.0, 0.0, 0.0]).unitVector())
        let perpendicularB = try #require(Vector([0.0, 1.0, 0.0]).unitVector())
        #expect(perpendicularA.cosineSimilarityForUnitVector(to: perpendicularB) == 0)

        let oppositeA = try #require(Vector([1.0, 0.0]).unitVector())
        let oppositeB = try #require(Vector([-1.0, 0.0]).unitVector())
        #expect(oppositeA.cosineSimilarityForUnitVector(to: oppositeB) == -1)

        let differentLengths = try unit.cosineSimilarityForUnitVector(to: #require(Vector([1.0, 0.0]).unitVector()))
